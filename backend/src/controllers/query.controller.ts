...
export const executeQuery = async (req: Request, res: Response) => {
  try {
    ...
    const executor = new WorkflowExecutor();
      // @ts-ignore
    const workflowConfig = workflow.configuration as WorkflowConfig;
...
export const getQueryHistory = async (req: Request, res: Response) => {
  try {
    ...
    if (status && ['success', 'error'].includes(status as string)) {
...
export const getQueryDetail = async (req: Request, res: Response) => {
  try {
...
async function updateWorkflowAnalytics(
  ...
  try {
    ...
    const newAvgLatency = 
      (analytics.avgLatency * analytics.successfulQueries + latency) / totalSuccessful;
    const newAvgConfidence = 
      (analytics.avgConfidence * analytics.successfulQueries + confidence) / totalSuccessful;
...
export const executeBatchQueries = async (req: Request, res: Response) => {
  try {
