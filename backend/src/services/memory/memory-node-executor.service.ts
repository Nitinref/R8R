...
export class MemoryNodeExecutor {
  ...
  async executeMemoryUpdate(
    ...
    try {
      ...
      if (config.importance.auto) {
        importance = this.calculateAutoImportance(
      ...
      if (config.deduplication.enabled) {
        ...
        if (duplicates.length > 0) {
          ...
          const shouldStore = await this.handleDuplicateMemories(
            duplicates,
            context,
            userId,
            importance,
            config.deduplication.mergeStrategy
          );
   ...
   */
  async executeMemorySummarize(
    config: MemorySummarizeConfig,
    context: MemoryExecutionContext,
    userId: string,
    workflowId: string
  ): Promise<MemoryExecutionContext> {
    try {
      ...
      for (const group of eligibleGroups) {
        try {
          const result = await this.memoryService.summarizeMemories(
            group.map(m => m.id),
            userId,
            config.strategy.preserveDetails
          );
  ...
  private calculateRerankScore(memory: MemoryMatch, query: string): number {
  ...
  private getEngagementScore(accessCount: number): number {
    // Logarithmic scaling for engagement
    return Math.min(Math.log10(accessCount + 1) / 2, 1);
  }

  private getLengthScore(content: string): number {
  ...
  private calculateAutoImportance(
    ...
    if (hasCode) importance += 0.15;

    const hasStructuredData = /\{.*\}/.test(answer) || /\[.*\]/.test(answer);
    if (hasStructuredData) importance += 0.1;

    const hasNumbers = /\d{3,}/.test(answer);
    if (hasNumbers) importance += 0.05;
    ...
    if (hasBulletPoints || hasNumberedList) importance += 0.05;
  ...
  private async handleDuplicateMemories(
    duplicates: MemoryMatch[],
    context: MemoryExecutionContext,
    userId: string,
    ...
    switch (strategy) {
      ...
      case 'merge_metadata':
        ...
        //   @ts-ignore
          mergeCount: (duplicates[0].metadata as any).mergeCount + 1 || 1
        });
        return false;
  ...
  private async mergeMemoryMetadata(memoryId: string, metadata: any): Promise<void> {
  ...
  private validateMemoryContent(query: string, answer: string): { valid: boolean; reasons: string[] } {
    const reasons: string[] = [];

    if (!query.trim()) reasons.push('Query is empty');
    if (!answer.trim()) reasons.push('Answer is empty');
    
    if (query.length < 3) reasons.push('Query too short');
    if (answer.length < 5) reasons.push('Answer too short');
    
    if (query.length > 10000) reasons.push('Query too long');
    if (answer.length > 20000) reasons.push('Answer too long');
  ...
  private classifyMemoryType(
    query: string,
    answer: string
  ): 'conversation' | 'fact' | 'preference' | 'decision' | 'instruction' | 'explanation' {
  ...
  private groupSimilarMemories(
    ...
    const groups: MemoryMatch[][] = [];
    const used = new Set<string>();

    for (const memory of memories) {
      if (used.has(memory.id)) continue;

      const group: MemoryMatch[] = [memory];
      used.add(memory.id);
  ...
  private calculateMemorySimilarity(m1: MemoryMatch, m2: MemoryMatch): number {
    let similarity = 0;
    ...
    const timeDiff = Math.abs(
      m1.metadata.timestamp.getTime() - m2.metadata.timestamp.getTime()
    );
  ...
  private async applyMemoryExpiration(userId: string, expirationDays: number): Promise<void> {
...
let memoryNodeExecutorInstance: MemoryNodeExecutor | null = null;

export function getMemoryNodeExecutor(): MemoryNodeExecutor {
