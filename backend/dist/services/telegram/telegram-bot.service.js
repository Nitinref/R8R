...
export class EnhancedTelegramBotService {
    ...
    async handleBuildWorkflow(msg) {
        ...
        if (!userId) {
            await this.safeSendMessage(chatId, '❌ Please login first using `/login <email> <password>` or `/link <api_key>`');
            return;
        }
        ...
        await this.safeSendMessage(chatId, `🏗️ *Workflow Builder*\n\n` +
    ...
    async handleNodeDetails(msg, nodeType) {
        const chatId = msg.chat.id;
        const rateLimit = this.isRateLimited(chatId);
        if (rateLimit.limited) {
            await this.safeSendMessage(chatId, rateLimit.message);
        ...
        if (!node) {
            await this.safeSendMessage(chatId, `❌ Node type "${nodeType}" not found.\n\nUse \`/nodes\` to see all available nodes.`);
            return;
        }
    ...
    // =====================================================
    async handleModeSelection(chatId, session, text) {
        const choice = text.trim();
        switch (choice) {
            case '1':
                session.mode = 'visual';
                session.step = 'name';
                await this.safeSendMessage(chatId, `🎨 *Visual Builder Mode*\n\n` +
            ...
            case '3':
                ...
                break;
            default:
                await this.safeSendMessage(chatId, `❌ Please choose 1, 2, or 3\n\n` +
                    `1️⃣ Visual Builder\n` +
                    `2️⃣ Guided Setup\n` +
                    `3️⃣ Templates`);
        }
    }
    async handleTemplateSelection(chatId, session, text) {
        if (!text) {
            await this.safeSendMessage(chatId, `📋 *Workflow Templates:*\n\n` +
        ...
        switch (text.trim()) {
            ...
            case '5':
                session.step = 'name';
                await this.safeSendMessage(chatId, `🔧 *Custom Workflow*\n\n` +
        ...
        session.step = 'name';
        await this.safeSendMessage(chatId, `✅ *${workflowType} Template Loaded*\n\n` +
            `Nodes: ${nodes.length}\n` +
    ...
    async handleWorkflowName(chatId, session, text) {
        ...
        await this.safeSendMessage(chatId, `✅ *Workflow name:* ${text}\n\n` +
    ...
    async handleWorkflowDescription(chatId, session, text) {
        ...
        if (session.nodes.size > 0) {
            ...
            await this.safeSendMessage(chatId, `*Step 3:* Advanced Configuration\n\n` +
    ...
    async showNodeSelectionMenu(chatId) {
    ...
    async handleNodeSelection(chatId, session, text) {
        if (text.toLowerCase() === 'done') {
            if (session.nodes.size === 0) {
                await this.safeSendMessage(chatId, '❌ You must add at least one node!\n\nSelect a node number to add.');
                return;
            }
        ...
        if (isNaN(nodeIndex) || nodeIndex < 0 || nodeIndex >= AVAILABLE_NODES.length) {
            await this.safeSendMessage(chatId, `❌ Invalid node number. Choose 1-${AVAILABLE_NODES.length} or type \`done\``);
            return;
        }
        ...
        await this.safeSendMessage(chatId, `✅ *Added: ${nodeTemplate.emoji} ${nodeTemplate.name}*\n\n` +
            `*Configure this node:*\n\n` +
    ...
    async handleNodeConfiguration(chatId, session, text) {
        ...
        if (text.toLowerCase() === 'keep' || text.toLowerCase() === 'done') {
            ...
            await this.safeSendMessage(chatId, `✅ *Node configured!*\n\n` +
        ...
        if (text.startsWith('edit ')) {
            ...
            if (parts.length < 2) {
                await this.safeSendMessage(chatId, '❌ Invalid format. Use: `edit <key> <value>`\n\nExample: `edit llm.temperature 0.5`');
                return;
            }
            const key = parts[0];
            ...
            try {
                ...
                await this.safeSendMessage(chatId, `✅ *Updated ${key}*\n\n` +
                    `New config:\n\`\`\`json\n${JSON.stringify(node.config, null, 2)}\n\`\`\`\n\n` +
                    `Continue editing or type \`done\``);
            }
            catch (error) {
                await this.safeSendMessage(chatId, `❌ Failed to update config: ${error.message}`);
            }
            return;
        }
        await this.safeSendMessage(chatId, '❌ Unknown command. Use:\n• `keep` - Keep defaults\n• `edit <key> <value>` - Modify config\n• `done` - Finish');
    }
    async showConnectionMenu(chatId, session) {
        const nodesList = Array.from(session.nodes.values());
        if (nodesList.length === 1) {
            // Single node - no connections needed
            session.step = 'advanced_config';
            await this.safeSendMessage(chatId, `✅ *Workflow structure complete!*\n\n` +
    ...
    async handleNodeConnection(chatId, session, text) {
        if (text.toLowerCase() === 'done') {
            if (session.nodes.size > 1 && session.pendingConnections.length === 0) {
                await this.safeSendMessage(chatId, '⚠️ No connections defined! Type `auto` for automatic connections or create manual connections.');
                return;
            }
            // Apply connections
            this.applyConnections(session);
            session.step = 'advanced_config';
            await this.safeSendMessage(chatId, `✅ *Connections applied!*\n\n` +
        ...
        if (text.toLowerCase() === 'auto') {
            ...
            const nodesList = Array.from(session.nodes.values()).sort((a, b) => (a.position || 0) - (b.position || 0));
            ...
            this.applyConnections(session);
            await this.safeSendMessage(chatId, `✅ *Auto-connected ${session.pendingConnections.length} node(s) sequentially!*\n\n` +
        ...
        if (text.startsWith('connect ')) {
            ...
            if (parts.length !== 2) {
                await this.safeSendMessage(chatId, '❌ Invalid format. Use: `connect <from> <to>`\n\nExample: `connect 1 2`');
                return;
            }
            ...
            if (isNaN(fromIndex) || isNaN(toIndex) ||
                ...
                await this.safeSendMessage(chatId, `❌ Invalid node indices. Choose between 1-${nodesList.length}`);
                return;
            }
            ...
            await this.safeSendMessage(chatId, `✅ *Connection added:* ${fromIndex + 1} → ${toIndex + 1}\n\n` +
    ...
    async handleAdvancedConfig(chatId, session, text) {
    ...
    async handleWorkflowConfirm(chatId, session, text) {
        ...
        if (text.toLowerCase() !== 'confirm') {
            ...
            const hasAnswerGen = Array.from(session.nodes.values()).some(n => n.type === 'answer_generation');
        ...
        try {
            ...
            this.cleanupSession(chatId);
            const nodesList = Array.from(session.nodes.values());
            await this.safeSendMessage(chatId, `✅ *Workflow Created Successfully!*\n\n` +
                `*Name:* ${workflow.name}\n` +
                `*ID:* \`${workflow.id}\`\n` +
            ...
            await this.safeSendMessage(chatId, '❌ Failed to create workflow. Please try again.');
        }
    }
    // =====================================================
    // VIEW WORKFLOW STRUCTURE
    // =====================================================
    async handleViewWorkflow(msg, workflowName) {
        const chatId = msg.chat.id;
        const userId = this.userMapping.get(chatId);
        const rateLimit = this.isRateLimited(chatId);
        if (rateLimit.limited) {
    ...
    // CLONE WORKFLOW
    // =====================================================
    async handleCloneWorkflow(msg, workflowName) {
        const chatId = msg.chat.id;
        const userId = this.userMapping.get(chatId);
        const rateLimit = this.isRateLimited(chatId);
        if (rateLimit.limited) {
        ...
        try {
            ...
            const config = clonedWorkflow.configuration;
            const steps = config.steps || [];
            await this.safeSendMessage(chatId, `✅ *Workflow Cloned Successfully!*\n\n` +
    ...
    visualizeWorkflowFlow(nodes) {
        ...
        const buildFlow = (nodeId, indent = '', isLast = true) => {
    ...
    async handleStart(msg) {
        ...
        // @ts-ignore
        await this.safeSendMessage(chatId, `🤖 *Welcome to R8R Workflow Bot!*\n\n` +
    ...
    async handleHelp(msg) {
        ...
        await this.safeSendMessage(chatId, `📚 *R8R Workflow Bot - Complete Guide*\n\n` +
    ...
    async handleRegister(msg, match) {
        ...
        if (!match || match.length < 4) {
            await this.safeSendMessage(chatId, '❌ *Usage:* `/register <email> <password> <name>`');
            return;
        }
        ...
        try {
            ...
            await this.safeSendMessage(chatId, `✅ *Registration Successful!*\n\n` +
                `Welcome, *${name}*!\n\n` +
                `🔑 *Your API Key:* \`${apiKey}\`\n\n` +
    ...
    async handleLogin(msg, match) {
        ...
        try {
            ...
            this.userMapping.set(chatId, user.id);
            await this.safeSendMessage(chatId, `✅ *Login Successful!*\n\n` +
    ...
    async handleLinkAccount(msg, match) {
        ...
        if (!match || !match[1]) {
            await this.safeSendMessage(chatId, '🔗 *Link Your Account*\n\n' +
        ...
        try {
            ...
            await this.safeSendMessage(chatId, `✅ *Account Linked Successfully!*\n\n` +
    ...
    async handleListWorkflows(msg) {
        ...
        try {
            ...
            if (workflows.length === 0) {
                await this.safeSendMessage(chatId, '📋 You have no workflows yet.\n\n' +
                    'Create one with:\n' +
                    '• `/build` - Visual builder\n' +
                    '• `/create` - Guided setup');
                return;
    ...
    async handleDeleteWorkflow(msg, workflowName) {
        const chatId = msg.chat.id;
        const userId = this.userMapping.get(chatId);
        const rateLimit = this.isRateLimited(chatId);
        if (rateLimit.limited) {
        ...
        try {
            ...
            await this.safeSendMessage(chatId, `🗑️ *Workflow Deleted Successfully!*\n\n` +
                `*Name:* ${workflow.name}\n` +
                `*ID:* ${workflow.id}\n\n` +
    ...
    async handleRunWorkflow(msg, workflowName) {
        ...
        try {
            ...
            this.resetSessionTimeout(chatId);
            await this.safeSendMessage(chatId, `🚀 *Ready to execute:* ${workflow.name}\n\n` +
    ...
    async handleDirectQuery(msg, query) {
        ...
        try {
            ...
            if (!workflow) {
                await this.safeSendMessage(chatId, '❌ No workflows found. Create one with:\n' +
                    '• `/build` - Visual builder\n' +
                    '• `/create` - Guided setup');
                return;
            }
    ...
    async executeWorkflowQuery(chatId, userId, workflowId, query) {
        try {
            ...
            const result = await this.workflowExecutor.executeWorkflow(workflow.configuration, query, userId);
            ...
            await this.safeSendMessage(chatId, '❌ Failed to execute workflow. Please check workflow configuration.');
        }
    }
    // =====================================================
    ...
    resetSessionTimeout(chatId) {
        ...
        const timeout = setTimeout(() => {
            ...
            this.safeSendMessage(chatId, '⏰ Session expired due to inactivity. Use `/build` or `/create` to start again.');
        }, 30 * 60 * 1000);
