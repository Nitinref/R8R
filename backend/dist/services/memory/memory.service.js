...
export class MemoryService {
    ...
    async storeMemory(request) {
        try {
            ...
            // Compress content
            const { compressedQuery, compressedResponse } = await this.compressMemoryContent(request.query, request.response);
    ...
    async retrieveMemories(request, config) {
    ...
    async hybridSearch(request, keywordWeight = 0.3) {
        const startTime = Date.now();
        try {
            ...
            const boostedMemories = await this.applyKeywordBoost(vectorResults.data.memories, request.query, keywordWeight);
            const totalTime = Date.now() - startTime;
            return {
                success: true,
    ...
    async updateImportance(request) {
        try {
    ...
    async summarizeMemories(memoryIds, userId, preserveDetails = false) {
        try {
            ...
            const combinedContent = memories.map((m, i) => `[${i + 1}] Query: ${m.query}\nResponse: ${m.response}`).join('\n\n');
            ...
            return {
                ...
                data: {
                    summarized: {
                        original: memories,
                        summary: summaryMemory.data?.stored
                    }
                }
            };
        }
    ...
    async cleanup(userId, maxMemories) {
        try {
    ...
    async deleteMemoryFromVectorDB(memoryId) {
        try {
            await this.qdrant.delete(this.collectionName, {
                points: [memoryId]
            });
    ...
    async getStats(userId, workflowId) {
        const where = { userId };
        if (workflowId)
        ...
        const totalChars = memoryStats.reduce((sum, mem) => sum + mem.query.length + mem.response.length, 0);
        return {
            total,
    ...
    async compressMemoryContent(query, response, maxLength = 2000) {
    ...
    validateMemoryRequest(request) {
        ...
        if (request.query.length > 10000)
            errors.push('Query too long (max 10000 chars)');
        if (request.response.length > 20000)
            errors.push('Response too long (max 20000 chars)');
    ...
    async applyKeywordBoost(memories, query, weight) {
