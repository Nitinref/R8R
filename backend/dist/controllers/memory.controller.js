 ...
 * POST /api/memory
 */
export const storeMemory = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            throw new AppError('Authentication required', 401);
        }
        const { workflowId, query, response, metadata } = req.body;
        if (!query || !response) {
 ...
 */
export const retrieveMemories = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            throw new AppError('Authentication required', 401);
        }
        const { workflowId, query, topK = 5, minScore = 0.7, filters } = req.body;
        if (!query) {
 ...
 */
export const updateMemoryImportance = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            throw new AppError('Authentication required', 401);
 ...
 */
export const summarizeMemories = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            throw new AppError('Authentication required', 401);
        ...
        const result = await memoryService.summarizeMemories(memoryIds, userId, preserveDetails);
        if (!result.success) {
...
export const getMemoryStats = async (req, res) => {
    try {
        ...
        const stats = await memoryService.getStats(userId, workflowId);
        res.json({
            success: true,
            data: stats
        });
 ...
 */
export const cleanupMemories = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            throw new AppError('Authentication required', 401);
...
export const searchMemories = async (req, res) => {
    try {
        ...
        const { query, workflowId, types, tags, minImportance, maxImportance, dateFrom, dateTo, limit = 20, offset = 0, sortBy = 'relevance' } = req.body;
        ...
        if (maxImportance !== undefined)
            filters.maxImportance = maxImportance;
        let result;
        if (query) {
